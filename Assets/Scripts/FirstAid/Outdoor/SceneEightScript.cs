...
public class SceneEightScript : MonoBehaviour
{
    ...
    {
        Option = 1;
        FindObjectOfType<DB>().UpdateLevelDone(8, 9);
        FindObjectOfType<DB>().UpdateLevelSCore(8, TimesSelected);
    ...
    private void ChoosingQue()
    {
        if (DialogFinished == true)
        {
            if (Option == 1)
            {
                FindObjectOfType<LevelLoader>().LoadNextLevel(9, TimesSelected);
            }
            else
            {
                FindObjectOfType<FirstAidMenu>().SetCindyPanel(false);
