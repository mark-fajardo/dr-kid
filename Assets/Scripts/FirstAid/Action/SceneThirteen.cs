...
public class SceneThirteen : MonoBehaviour
{
    ...
    {
        Option = 3;
        FindObjectOfType<DB>().UpdateLevelDone(13, 14);
        FindObjectOfType<DB>().UpdateLevelSCore(13, TimesSelected);
        ShowVideo();
    }
    ...
    private void ChoosingQue()
    {
        if (DialogFinished == true)
        {
            if (Option == 3)
            {
                FindObjectOfType<LevelLoader>().LoadNextLevel(17, TimesSelected);
            }
            else
            {
                FindObjectOfType<FirstAidMenu>().SetCindyPanel(false);
